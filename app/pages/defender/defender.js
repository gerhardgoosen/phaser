...
angular.module('myPhaserApp.defender', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
    $routeProvider.when('/defender', {
        templateUrl: 'pages/defender/defender.html',
        controller: 'DefenderCtrl'
    });
}])

.controller('DefenderCtrl', [function() {
    // scope.$on('$destroy', function() {
    //     game.destroy(); // Clean up the game when we leave this scope
    // });

    console.log('Defender Control Ready ==>');
    ...
    //Create a new game instance and assign it to the 'gameArea' div
    var game = new Phaser.Game(width, height, Phaser.AUTO, 'defender', {
        preload: preload,
        create: create,
        update: update
    });
    ...
    function preload() {
        game.load.image('player', 'pages/defender/assets/defender/ship.png');
        game.load.image('star', 'game/assets/demoscene/star2.png');
        game.load.image('baddie', 'game/assets/sprites/space-baddie.png');
        game.load.atlas('lazer', 'pages/defender/assets/defender/laser.png', 'pages/defender/assets/defender/laser.json');
